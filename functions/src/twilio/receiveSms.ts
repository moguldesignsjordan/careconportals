...
export const receiveSms = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const messageRef = await db.collection("smsMessages").add({
      ...
      status: "received",
      sentBy: null, // Inbound - no portal sender
      projectId: null,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      metadata: {
    ...
    // 4. Create in-app notification for admins
    // Get all admin users to notify them of inbound SMS
    ...
    console.log(`SMS stored: ${messageRef.id} in conversation ${conversationId}`);
 
    // 5. Return TwiML response (empty = no auto-reply)
...
// If you need to handle images/media sent via MMS:
export const receiveMms = functions.https.onRequest(async (req, res) => {
  ...
  if (numMedia === 0) {
    // No media - redirect to standard SMS handler
    // In practice, Twilio sends both to the same webhook
  ...
  console.log(`MMS received from ${fromFormatted} with ${numMedia} attachments`);
  ...
  try {
    ...
    // Store message with media references
